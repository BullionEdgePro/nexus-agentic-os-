...
| 13 | Marketplace | 🟡 **Built to the egress policy decided 2026-08-15: NOTHING LEAVES.** A catalogue of templates, procedures and knowledge packs a business INSTALLS and never contributes to. The boundary is the absence of a path rather than a rule someone must remember: `catalog_items` has no `organization_id` and no foreign key to any tenant table, so there is nowhere one business's material could be recorded — which is what Juris Prime Legal and ABR, two law firms on one number, are owed. Operator-only browse and install at `/deck/catalogue`; installed version is copied, not referenced, and one live install per pack is a partial unique index rather than an application check. **Installing does not yet change what a customer hears** — no payload is wired into the live agent, and the page says so instead of offering a switch that would lie |
