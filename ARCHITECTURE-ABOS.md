...
| **Waiting on a decision** (⛔) | **None.** Both refusals were flipped on 2026-08-25 at the owner's explicit and repeated request, and both were flipped deliberately rather than quietly: F14's judgement moved into five rules, each proved against a procedure that breaks it, with a human's OFF final and every activation announced by the 21st operator; F7's board ships without the automations, because the operator layer already watches those rows. The reasons the refusals gave are preserved in both rows above, because a decision reversed is worth reading beside the argument it overruled |
