...
| 8 | Operators | 🟡 **Built, and §2.3 answered by construction rather than by decision.** **Fourteen** operators sweep every business every 10 minutes and call no model at all — this row said five until 2026-08-17, which was true when written. The first five were customer waiting for a reply, overdue follow-up, unowned follow-up, failing knowledge source, and an agent with almost nothing to answer from; the eight since each watch a failure a newer feature made possible, the most recent being `intent-unclassified`, `handover-abandoned`, `retrieval-unavailable` — which migration 047 had to be written carefully around, because the keyword fallback stops replies being recorded as failures and would otherwise have emptied the predicate that operator sweeps on — and `delivery-failing`, which could not have existed before migration 048 because the fact it watches was not recorded anywhere. The fifth exists because ABR's five indexed passages were found by counting rows by hand: `broken-knowledge` watches sources that FAIL, and a business with too few working sources has nothing wrong with it in that sense. The design property that matters is that a finding can be **retracted**: each pass computes the complete truth and reconciles, so the list shrinks as well as grows. Paid inference remains an open choice, now additive rather than blocking |
