...
**Status: 2026-08-13.** Every text-generating call runs on Anthropic (`claude-sonnet-5` for agents, `claude-haiku-4-5` for short helpers); `gemini-embedding-001` remains for embeddings, which Anthropic does not offer. Zipicka has one person on its rota, so escalation reaches somebody for the first time. Phases 0, 2 and 3 shipped and verified. Feature 12 is
...
| **2 — Knowledge** | Schema + chunker + Gemini embeddings + citation-bearing retrieval; URL connector with SSRF guard; cross-page boilerplate stripping; 6-hourly re-indexing; **393 live chunks across all five businesses**, retrieval verified against real customer questions |
...
copilot questions (it said six), 69 migrations, twelve gates, 1314 tests. The eleventh, added 2026-08-24, is `backup-check`, and it is the only one that says nothing about whether the deploy works — it asks whether the thing that gets you back if it does not is still standing. `backup-db.sh` had been restoring every nightly dump into a scratch database since it was written, and writing the answer to a log no script, gate, operator or endpoint had ever opened. The twelfth, added 2026-08-26, is `serving-check`, and it is the only one that asks from outside. The other eleven run inside the worker container or against local files, so every one of them would pass on a platform nobody could reach — a wedged API, a misrouted Caddy, an expired certificate. The header of `verify-all.sh` had asserted for months that "every other gate talks to the API", which is why the hole lasted: it was written down as covered. `serving-check` also calls `/health/jobs`, an endpoint built to notice a dead operator sweep that nothing in the repository had ever called.

### The vocabulary, which is the point of this revision

Eight rows here carried 🟡, and somebody reading that reasonably concluded there
were eight unfinished features. Exactly one of them was genuinely unfinished, and
it shipped on 2026-08-22. What sat under the same symbol was three features
waiting on data and two deliberate refusals — neither of which is work, and the
second of which is somebody's decision to make. 🟡 was doing four different jobs
at once — genuinely unbuilt, built-and-waiting-for-data, deliberately refused,
and deferred-because-nobody-asked — and collapsing those into one symbol told
the owner their product was half-finished when it is not.

| | Means |
|---|---|
| ✅ | Built and in use |
| ⏸ | **Built and correct; inert until there is data.** Not debt. The output of an empty system IS the empty output, and several rows here say so explicitly |
| ⛔ | **Deliberately not built**, with the reason recorded. Flipping one of these is a product decision, not a task |
| 🟡 | Genuinely unbuilt scope. **No row carries this today** |

| # | Feature | Status |
|---|---|---|
| 1 | Employee Agent Layer | ✅ **Complete 2026-08-25.** Roster UI, the handover brief, and calendar presence — the last open sub-item, which had said "needs a calendar integration" since this platform started. It does not need one: a secret ICS link is the same fact by a shorter road, published by Google, Outlook, Apple and Fastmail alike, read-only by construction, needing no credentials on this side and revoked by rotating the link. The distinction it buys is the whole point — **a rota says when somebody is meant to be available; a calendar says whether they are**, and `hasStaffOnShift` decides whether the agent promises a customer a person. Promising the first where the second says otherwise is the §9.5 failure through a fourth door. A meeting inside working hours makes somebody *busy*, exactly like a scheduled break; it can never make them *available* outside them, because an evening meeting is evidence somebody is occupied and never evidence they are at work. A person's own manual override outranks their diary, since they are looking at both and this is not. `resolvePresence` stays pure — the busy state is passed in, asserted by a test, because it runs on every inbound message. **The parser refuses to guess.** DAILY and WEEKLY recurrence is expanded, including INTERVAL, COUNT, UNTIL, BYDAY and EXDATE; MONTHLY and YEARLY are **counted and reported**, never dropped, because a calendar that half-parses is worse than one that does not parse at all — the missing half is invisible and the person looks free at exactly the times nothing could see. Folded lines, floating times in the employee's own zone, named zones resolved through Intl so they stay right across a clock change, TRANSP:TRANSPARENT and STATUS:CANCELLED all handled; 22 tests. **The link is treated as the credential it is:** whoever holds it reads every event title and attendee with no sign-in, so it goes in and never comes back out — the record carries the host, the panel never pre-fills, the log names the host, and the one function returning the URL is named for it and called only by the sync. Pasted addresses go through the same SSRF guard as knowledge URLs, at paste time. A failed sync records why and **leaves the busy time standing**: stale is wrong in the safe direction, absent is wrong in the direction that ends with nobody answering. Fifteen-minute cycle, watched by `schedule-stalled` with the second-tightest tolerance in the platform, because a dead calendar sync looks like nothing at all from every screen. **Corrected 2026-08-18:** the agent config was read as the number's OWNER, so on a shared number every customer routed to a serving business hit "No active agent configured" and received nothing at all — four of five businesses, for as long as the switchboard has existed. Fourth instance of the shared-number RLS trap; `withServingTenant` in `loadActiveAgentConfig`, and the bare `return` that followed it now sends the fallback and records the outcome |
| 2 | Knowledge Ingestion | ✅ Core, URL connector, scheduled re-index, and a management screen. Remaining connectors phased by real demand. **Corrected 2026-08-18:** retrieval read `knowledge_chunks` for the SERVING business from inside the number owner's transaction, so on a shared number it returned nothing for four of five businesses and every routed customer's question would have been answered "I'll check with a colleague". Hidden until that day by a worse bug that stopped routed customers reaching retrieval at all. Fifth instance of the shared-number trap; `shared-number-check` now exists to catch the sixth. **Retrieval no longer stops dead when the embedding provider does** (migration 047): semantic search remains the only ranker and a miss is still a refusal, but where the provider is *unreachable* Postgres full-text answers from the same chunks and the reply is recorded `degraded` rather than `hit`, so the outage stays visible instead of hiding inside its own mitigation. Worth **13 of `retrieval-check`'s 18 probes** against semantic search's 18 — the comparison that matters being 13 against zero, which is what an outage got before. The five it misses are confidently wrong rather than near misses and outrank correct hits, so there is no score floor; the excerpts are labelled as keyword matches and the model is told to ignore any that merely share a word with the question |
| 3 | Lead Intelligence | ✅ Rules-based, EN + AR. **"Model second once labels exist" was a condition nobody could reach until 2026-08-25**, because nothing in this platform ever produced a label: the scorer decided, `lead_assessments` recorded what it decided, and there was nowhere to say whether it was right. What that cost is not the missing model — it is that a rules scorer nobody has ever checked is indistinguishable from a good one, and it has been ranking real customers for months. So a lead can now be marked **worth it / not worth it** where it is already listed. Binary on purpose: "what should the priority have been" asks somebody to do the scorer's job from memory a fortnight later, and the answers would be noise, whereas "was this worth your time" is answerable in one click by whoever handled it — and is exactly what a model would train on. An optional outcome (`won`, `lost`, `no_reply`, `not_a_lead`) is refused unless it is one of those four, because a column of near-synonyms is a column nothing can count. One label per assessment, attributed to a person, since this is training data for something that will later decide which customers get attention. **Two numbers, never averaged.** A ranking fails in two directions that are not equally bad: a false alarm costs somebody a minute, a miss costs the business the lead — and a single "accuracy" looks fine while every miss is a customer who went elsewhere. Each refuses below eight labels on its own side, and the refusal names which side is short and by how many, since "not enough data" tells nobody whether that means two more or two hundred. A side that is ready is published; the other still says what is missing. The maths is pure and separate from the fetching, so the refusal is testable without a database. Migration 067 |
| 4 | Campaign Engine | ✅ **Both paths verified.** Database path verified end to end by `schema-check.ts` — draft, audience, recipients, status. Two bugs it found first: a non-existent column in the audience count, and a parameter Postgres could not type, which meant a draft could never be created. The Meta path is now **proven against the live service** (2026-08-13): all five businesses have Active templates at Meta, and `syncAllTemplates()` reached the real API and reconciled 7 approved templates per business, retiring none. Nothing was sent — this is the read side, which had never once run against Meta. Business verification is in review, which lifts messaging limits but does not block drafting |
| 5 | Neural Brain | 🟢 **Complete.** Pooled outcomes across every business, the gate holding: redaction fails closed and only structured fields cross a tenant boundary, never prose. `rollUpSharedPatterns` recomputes hourly; `/deck/quality` now shows it, coverage before patterns, refusing to draw any number it did not receive. **Pooled guidance reaches a reply** — the one thing here that shapes what a customer is told without a person at that business approving it, so it is fenced: own procedure always wins, the pool's own two-tenant and 20-sample floors are reused rather than re-declared, a 0.5 escalation floor on top, no figures and no mention of other businesses in the prompt, and no `procedureId` stamped because no procedure was applied. `intent-unclassified` (12th operator) watches the one number whose silence used to look like a quiet week. **Inert until traffic, and the floor is now provably doing work rather than having nothing to work on.** Re-measured 2026-08-25: ONE pattern row exists — `knowledge_lookup`/`en`, **2 samples from 1 tenant** — and the two-tenants-and-twenty floor correctly excludes it, so 0 patterns are shareable and no pooled guidance has ever reached a reply. The earlier note said "0 patterns", which was true of the shareable set and read as though the table were empty; a floor that has never had a candidate to reject is not the same claim as one that rejects the candidate it has. Originally verified by seeding one in a rolled-back transaction |
| 6 | PAUL v2 | ✅ **Built 2026-08-22, and the number that justifies it is eleven** — the times one defect has been found here, the eleventh by somebody who had spent that week cataloguing the other ten. This repository already recorded its own defects unusually well; nothing read the record. Three files warn in their headers that a pipeline swallows its exit code, and the mistake was then made three times in one day *by the author of two of those warnings*. So: a register of eight classes with their mechanism, instance count and evidence; detectors that run on every commit; and a rule with teeth — **a class that has recurred three times must name something that catches it, or state in writing why nothing can**, or the suite goes red. `whyUncoverable` is not a shrug; one class uses it, because a mistake made at a shell prompt leaves nothing in the repository to scan. The gate also asserts each detector *examined something*, since zero findings and zero checks look identical from outside — the prose detector's first version resolved every path three levels too deep and reported a clean tree while checking two assertions out of 219. The detector that matters catches a test **satisfied by a comment**: proven by deleting the tenant scoping from the inbox query and leaving the requirement in prose, whereupon all 15 tests across two files stayed green and the detector named both. `recurrence-harvest.mjs` closes the loop by re-reading the history for counts that have decayed, and **proposes without writing**, for the same reason the procedure inference never activates. **And it earns its place on the second day.** A third detector asks whether the markers these tests search for still exist in the files they search: 244 markers, 198 of them bounding a slice, ten guarding against `-1`. One was stale — `handover-brief` asserted the AI is paused BEFORE the handover summary is built (*"ordering is the whole safety property"*, says its own comment, because a slow model must not delay the pause), searching for a call that gained a required argument in `ae0ec7024`. The assertion had become `-1 < 20538`, which is TRUE: it would have passed with the handoff after the brief, or deleted outright, and 1034 tests stayed green over it. Repaired to assert the marker was found before comparing, and verified by swapping the two calls. Developer tooling, not product |
| 7 | Workspace | ✅ **Board shipped 2026-08-25 at the owner's request.** The scoped slice was already built: follow-ups tied to a conversation, owned, dated, raisable from the inbox. The board is at `/deck/board`, and its columns are the decision: **Overdue, Today, Later, Done** — WHEN, not state. A kanban over `open/done/cancelled` would have been useless, because two of the three are terminal and every live commitment lands in one pile. Dragging changes a real field, which is why `rescheduleTask` had to exist at all: `due_at` could previously only be set at creation, so the most ordinary edit to a follow-up had no door. Overdue is the SERVER's verdict (`isOverdue`, whose own comment forbids recomputing it in the browser); Today-vs-Later is local, and correctly so. Nothing drags out of Done — reopening undoes an accountability record — enforced both in the card and in the writer. Views are business and owner, per browser. **Automations shipped the same day, and the design is the refusal of the obvious build.** A rules engine watching the tasks table would be a second evaluator standing beside `overdue-followup` and `unowned-followup` — two pieces of code answering one question, and the day they disagree one of them is wrong on a screen nobody is comparing. So a rule reacts to a FINDING the sweep has already raised, and the operators stay the only eyes. Two actions, both of them things a colleague does: give somebody the follow-up, or write one down. **Nothing here can say anything to a customer** — that would be a wider grant than the reply path holds itself to, and it is enforced by the shape rather than by care: `AUTOMATION_ACTIONS` has two entries and the action-to-finding pairs are an allow-list, so an action is refused on a finding whose subject it could not act on rather than acting on the wrong row. A finding STANDS until it stops being true, so it is present in six sweeps an hour — which is why the claim row is written *before* the act and only the writer that won a unique index proceeds, making a lost race the normal case rather than an error, and why `automation_runs.finding_id` is deliberately not a foreign key: a retracted finding must not take the record of what was already done with it. A failed act is recorded and never retried, because an automation that cannot assign since the person has left will not start working in ten minutes, and a retry writes the same sentence into the log six times an hour. `procedure-switched-on` is excluded from every trigger list — a rule reacting to the platform's own automatic act (F14) is how a loop starts. The runner reads the findings itself rather than taking the sweep's dispatch payload, which carries no subject on purpose because a finding's title names a customer. Migration 064; the panel sits under the board. **0 rules exist in production**, and that is the default rather than a gap: everything the checks find waits for a person until somebody switches a rule on, and the empty state says exactly that. ⛔ was the previous state: **Deferred, not started, and that is the decision.** The scoped slice is built: follow-ups tied to a conversation, owned, dated, raisable from the inbox — and they now travel back to the customer, reaching both the agent's context and the handover brief when that person messages again. Boards, views and automations are the months, and none is asked for yet |
| 8 | Operators | ✅ **Built. Twenty-one** operators sweep every business every 10 minutes and call no model at all — this row said five until 2026-08-17, which was true when written. The first five were customer waiting for a reply, overdue follow-up, unowned follow-up, failing knowledge source, and an agent with almost nothing to answer from; the sixteen since each watch a failure a newer feature made possible — the newest is `procedure-switched-on`, which is F14 saying out loud that this platform switched a procedure on by itself, an `info` finding computed from state so it retracts the moment a person reviews that procedure — and before it `intent-unclassified`, `handover-abandoned`, `retrieval-unavailable` — which migration 047 had to be written carefully around, because the keyword fallback stops replies being recorded as failures and would otherwise have emptied the predicate that operator sweeps on — `delivery-failing`, which could not have existed before migration 048 because the fact it watches was not recorded anywhere, and `agent-unavailable` — the alarm that was missing both times the model provider actually failed here, and which needed migration 049 first for the same reason: a reply that threw jumped past the metric write, so the outages were not merely unmonitored, they were unrecorded. The sixteenth, `schedule-stalled`, is the one that admits what it cannot see: it runs inside the sweep, so it watches six of the seven scheduled jobs and deliberately not the sweep itself — a check that would pass in every case where it could conceivably be needed. `GET /health/jobs` covers that one from outside (migration 050). The fifth exists because ABR's five indexed passages were found by counting rows by hand: `broken-knowledge` watches sources that FAIL, and a business with too few working sources has nothing wrong with it in that sense. The design property that matters is that a finding can be **retracted**: each pass computes the complete truth and reconciles, so the list shrinks as well as grows. The second design property, added 2026-08-25, is that an **acceptance has an end**. Migration 061 let a person accept a finding that is true and does not need action, lapsing it when the finding resolved and came back — right for every condition that ENDS, and a permanent silent mute for every condition that does not. Its own test warned about exactly that in its opening paragraph and then closed only half of it. Production held four standing findings on 2026-08-25 and all four were accepted, the urgent one at roughly 118 hours of a customer waiting; it read 142.3 a day later, still climbing, and could never re-surface. So an acceptance now names its length — a day, a week, a month, with **no forever on the menu** — the sweep clears it once that passes, and the deck shows when each one comes back. The four dismissal columns lapse under one shared predicate, asserted field-by-field, because splitting them leaves a row accepted by nobody or accepted with no end. Paid inference remains an open choice, now additive rather than blocking |
| 9 | Command Center | ⏸ Deck on live queries, plus team activity and agent quality. One rollup table exists (daily quality); the overview still aggregates live. **The customer record shipped 2026-08-25, and its absence is the plainest gap this platform had:** fourteen screens and not one of them showed a person. You could read a conversation, a follow-up, a booking, a lead score — every artefact a customer produces — and never the customer, whose history sat across five tables with nothing that put it back together. It matters more than convenience because `contact_memory` holds what this platform has REMEMBERED about somebody, inferred from their messages, and its only readers were two verification scripts. Nobody could see what was held. `forgetContact` — written because "delete what you hold about me is a request a customer can make, and an answer of 'we would have to write some code' is not one" — had no caller outside a script, so the honest answer to that request was still "we would have to run something". Both are now on a screen: the remembered text is shown in the words it is held in, and erasing it asks first and says what survives (the conversations, which are the business's own record). **Scoped on the SERVING business throughout**, which is the thirteenth outing for this platform's signature defect: a contact row belongs to the number's OWNER, so `where organization_id = $1` offers Zipicka people who only ever spoke to Juris Prime and offers Juris Prime nobody. `contactServedBy` defines that predicate once and a test asserts no reader rolls its own — the inbox and the search drifted apart on exactly this once already. Two competing law firms share this number, so another firm's customer is a 404 rather than a 403, and only the other businesses' NAMES cross, never their conversations. **Export shipped the same day, and its absence was the plainer half:** there was none anywhere in this product — the only file it would give you was a QR code — so a business could ask this platform to FORGET a customer and could not obtain one row of its own records. Customers and every message as CSV; one person's whole record as JSON, which is what a subject access request actually asks for, sitting beside the erase button because the two are one request asked twice. **The CSV is a security surface and that is most of the work:** every spreadsheet evaluates a cell beginning with `=`, `+`, `-`, `@`, tab or carriage return, and every cell here carries text a stranger typed into WhatsApp — so the export is the door a formula comes back through. Such cells are prefixed with an apostrophe rather than stripped, because rewriting what a customer said to make a file safe is the worse trade. Plus RFC 4180 quoting, and a UTF-8 BOM with CRLF endings, without which Excel renders every Arabic name as mojibake on a platform that answers in Arabic. Scoped through `contactServedBy` like the screens; the bulk export says whether a summary is HELD and never what it says; truncation is reported in a header and shown as an amber notice, because a partial record that looks complete is worse than none. **Verified against production: 0 messages currently begin with a formula character, so the guard is correct and has nothing to act on yet** |
| 10 | Memory | ⏸ **Procedural IS formalised** — this row said otherwise until 2026-08-17. Semantic + episodic (per-business contact memory, expiring, forgettable) plus procedural: `procedures` with a nightly inference writer that proposes and never activates, a review screen at `/deck/procedures`, evidence recomputed rather than incremented, and the catalogue able to install one. The gap is not the feature, it is use — **0 procedures exist across all five businesses, so no customer message has ever met one** |
...
| 13 | Marketplace | 🟢 **Complete end to end, to the egress policy decided 2026-08-15: NOTHING LEAVES.** A catalogue of wording, methods and knowledge a business INSTALLS and never contributes to. The boundary is the absence of a path rather than a rule someone must remember: `catalog_items` has no `organization_id` and no foreign key to any tenant table, so there is nowhere one business's material could be recorded — which is what Juris Prime Legal and ABR, two law firms on one number, are owed. Operator-only at `/deck/catalogue`. **Four decisions, never one flow:** install (records the choice and the version), add (materialises into the business — a procedure into How we answer *switched off*, wording into What we say *switched off*, a knowledge pack indexed and therefore live at once, which the page says out loud), switch on (only ever on those screens, where a person can see what else answers that situation), and take an update (v1 stays v1 until somebody presses the button — a live procedure is *proposed to* via `proposed_steps`, never edited; a live phrase is refused until switched off; anything rewritten by hand is refused outright). Installed version is copied not referenced; one live install per pack, one procedure per install, and one active phrase per moment are all partial unique indexes rather than application checks. **Caveat: 0 businesses have installed anything, and the only published knowledge pack is `guidance_only`, so the pack→`knowledge_chunks` ingest branch has never run against a real pack in production** |
| 14 | Self-improving AI | ✅ **Automatic action shipped 2026-08-25, at the owner's explicit and repeated request, and the refusal it replaces is worth reading first: "the judgement of whether a rate is wrong belongs to someone who knows the business". That judgement now lives in `autoActivationDecision` as five rules, each proved in the suite against a procedure that breaks it. (1) Only what the platform inferred — an operator's own draft is theirs to switch on. (2) **A HUMAN'S OFF IS FINAL**: if a person reviewed it and left it inactive, no amount of later evidence overrules them, which is the rule that makes the rest tolerable and is proved twice. (3) A dismissal holds until the evidence has DOUBLED. (4) A floor of twenty well-handled conversations — the shared brain's number, not a second one invented for the same judgement. (5) It is never silent: `procedure-switched-on` (21st operator) reports every method the agent follows that nobody has approved, computed from state so it RETRACTS the moment somebody looks. It never deactivates and never edits steps: an activation that turns out wrong is one click from undone, a deactivation nobody noticed leaves the agent quietly worse with no trace. On today's data it correctly does nothing — 0 procedures exist. ⛔ was the previous state: Escalation, containment and correction rate from human actions, plus escalation hotspots pointing at the knowledge screen. **The rates these are computed from became measurable on 2026-08-17** — migration 049. Until then a reply that failed left no `conversation_metrics` row at all, so every rate here was computed over replies the model managed to produce: 12 rows, all `ai_agent`, beside 4 fallback messages that were absent from the table. An AI resolution rate of 100% by construction. Automatic action is deliberately not taken — the judgement of whether a rate is wrong belongs to someone who knows the business |
| 15 | BI Copilot | ✅ Built on live queries and the quality rollups. **Ten** reviewed questions; the model routes and never writes SQL, because one database holds five companies' conversations and a generated WHERE clause is a leak waiting for the right prompt. The four added 2026-08-22 answer what this platform spent a week proving matters: who is waiting for a reply right now, what the operators are reporting, how much the agent has to answer from, and what is in the diary. `waiting_now` applies the same cold-pitch suppression the operators screen applies, and says how many it set aside — getting that wrong first is recorded in §8. Adding a capability still means adding a question on purpose, which is the architecture rather than a gap |

### What is actually left, and who it is waiting on

| | |
|---|---|
| **Waiting on data, not on code** (⏸) | Command Center rollups, Memory, Predictive BI, and by their own rows Neural Brain and Marketplace. Every one is built and correct; each needs traffic, procedures, installs or history that does not exist yet. Writing more code here would not move any of them |
| **Waiting on a decision** (⛔) | **None.** Both refusals were flipped on 2026-08-25 at the owner's explicit and repeated request, and both were flipped deliberately rather than quietly: F14's judgement moved into five rules, each proved against a procedure that breaks it, with a human's OFF final and every activation announced by the 21st operator; F7's automations ship as hands rather than a second pair of eyes, reacting to a finding the sweep has already raised, unable to say anything to a customer, and switched on by a named person one rule at a time. The reasons the refusals gave are preserved in both rows above, because a decision reversed is worth reading beside the argument it overruled |
| **Genuinely unbuilt** | **Nothing, and the open sub-items are closed too.** F6 was the last whole row and shipped 2026-08-22. The four remainders inside ✅ rows went on 2026-08-25: calendar presence (F1), the file connector (F2), lead labelling (F3), and the one genuine gap this platform had left — **nobody was ever told when work was assigned to them**, which mattered more once a rule could assign a follow-up at three in the morning. One thing is deliberately not built and is named rather than buried: **out-of-band delivery of that notification**. WhatsApp will not carry an unsolicited message to a staff member without a template Meta has approved, and approving one is the owner's to do. Building a sender nothing can exercise is how a feature ships broken |
| **Waiting on a person** | The four website edits, `rclone` credentials, the admin password reset, an alert destination, and now a WhatsApp template for notifying staff. None needs engineering, and the first is still the only one that moves the actual constraint. Two more are simply *inputs*: a published iCal link per person makes calendar presence live, and marking eight leads either way makes the scorer judgeable |

**The constraint is not engineering.** Re-measured against production on
2026-08-25: 0 bookings, 0 tasks, 0 procedures, 0 broadcasts ever sent, and 0
catalogue installs.

And the shape of what little traffic there is explains most of the ⏸ rows at a
stroke. `conversation_metrics` holds **19 rows in total**. Eleven of them are
`inbound_pitch` — data brokers selling property leads — six are `unknown`, and
two are `knowledge_lookup`. Across all nineteen: **0 escalations and 0 retrieval
misses.**

That is why Self-improving AI has nothing to improve from and why its escalation
hotspot table does not render at all: a ranking of what most often reaches a
person, over a set where nothing has ever reached a person. It is not a gap in
the feature. Building more of it today would be adding columns to a table with
no rows.
...
hitting them. Not one of the five businesses' websites points at the number the
agent answers on.
...
| A safety check run with the evidence removed | The queue processor called the hallucination judge with the draft and the conversation history — and **no `ragContext` at all**. Every fact the agent had correctly retrieved therefore looked, to the judge, like an assertion supported by nothing, biasing it toward *high* on precisely the replies that did their job. The judge was not broken and the agent was not wrong; the two were introduced to each other with the evidence withheld |
| A judge that was never told who was speaking | Juris Prime Legal's agent declined to give legal advice, offered a consultation, and scored **high**: *"references 'Juris Prime Legal' as a service provider … not mentioned anywhere in the conversation history or retrieved context."* Correct on its own terms. High escalates for every strict tenant, so this alone would have blocked nearly every reply those three businesses produce — a right answer, withheld, replaced by a promise of a human |
| A business's own public inbox read as a data leak | Zipicka's agent answered a returns question completely and told the customer to email `marketing@zipicka.com` — the address printed on Zipicka's own refund-policy page, which is where it read it. The PII scanner flagged it, and **a PII match escalates for every tenant, tolerant ones included**. The same judge call simultaneously called that email "directly supported by the retrieved knowledge-base context". Two checks looking at one string, disagreeing, and the stricter one winning |
...
| A default standing in for a verdict | The Anthropic key has no credit, so every governance judge call throws and returns `risk: "medium"` — which is not a verdict, it is the absence of one, and no consumer can tell them apart. `shouldEscalateReply` escalates on medium for every tenant outside the tolerant allowlist, so **juris-prime, juris-prime-legal and abr had been escalating every reply they generated** (and with an empty rota, delivering the no-staff fallback instead of the grounded answer the agent wrote), while **zipicka and sfs-international sent every reply checked by nothing**. The deck reports "hallucination risk medium", which reads exactly like a judge that ran. Found by generating one reply per business and looking at the verdict beside it — the first time anybody had |
| Placeholder text indexed as knowledge | SFS's agent held four passages of Lorem ipsum it could retrieve and cite. `/privacy/` was an unreplaced Houzez theme page — whole body filler, five indexed passages — while the genuine policy at `/privacy-policy/` was not indexed at all. The curation excludes theme leftovers by URL (`grid-full-width`, `with-parallax`), which cannot work here: `/privacy/` is exactly what a real privacy policy is called. **An address cannot tell you whether the words underneath it mean anything.** Nothing was broken: the source fetched, chunked, embedded and reported healthy |
