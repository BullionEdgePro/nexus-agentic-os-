...
| 5 | Neural Brain | 🟡 **The store IS built** — this row said otherwise until 2026-08-17. `shared_patterns` exists, `rollUpSharedPatterns` runs on the hourly quality rollup, and the gate holds: redaction fails closed and what crosses a tenant boundary is an allow-list of structured fields, never prose. The real gaps are two. **Nothing in the web app shows it** — `getBrainStatus`, `getIntentCoverage` and `getSharedGuidance` are reachable only through `/api/quality`, so a pool nobody can see. And it is **nearly empty by honest arithmetic**: 1 pattern, biggest sample count 2, 0 at the 20-sample threshold, because four of five businesses have never had a customer. The threshold is doing its job; there is simply not enough traffic yet |
